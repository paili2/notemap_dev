...
export async function getImageUrlFromRef(
  ...
  try {
    ...
    if (ref.idbKey.startsWith(URL_PREFIX)) {
      // http(s) 원격 URL 은 그대로 사용
      return ref.idbKey.slice(URL_PREFIX.length);
    }
...
export async function removeImageRef(ref: ImageRef) {
  ...
  if (ref.idbKey.startsWith(URL_PREFIX)) return; // 원격 URL은 삭제 없음
